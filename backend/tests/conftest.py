...
# Tests must not boot the full multi-agent runtime on every app restart:
# the app is a process-wide singleton and TestClient restarts it repeatedly
# across a session. Re-initialising the agent runtime/brain on each restart
# accumulates global state and eventually raises RecursionError at startup.
# Production keeps autostart on; tests start agents explicitly via fixtures.
os.environ.setdefault("PROCUREFLOW_START_AGENTS_ON_STARTUP", "0")
